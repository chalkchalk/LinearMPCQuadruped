...
GazeboDog::GazeboDog(ros::NodeHandle &nh) : nh_(nh), info_to_ros_(nh)
{
    ...
    quadruped_cmd_sub_ = nh_.subscribe("/quadruped_cmd", 100, &GazeboDog::quadruped_cmd_callback, this);
    ROS_INFO("Using robot: %s", robot_name_.c_str());
    pub_joint_cmd[0] = nh_.advertise<unitree_legged_msgs::MotorCmd>(robot_name_ + "_gazebo" + "/FL_hip_controller/command", 1);
    pub_joint_cmd[1] = nh_.advertise<unitree_legged_msgs::MotorCmd>(robot_name_ + "_gazebo" + "/FL_thigh_controller/command", 1);
    pub_joint_cmd[2] = nh_.advertise<unitree_legged_msgs::MotorCmd>(robot_name_ + "_gazebo" + "/FL_calf_controller/command", 1);
    pub_joint_cmd[3] = nh_.advertise<unitree_legged_msgs::MotorCmd>(robot_name_ + "_gazebo" + "/FR_hip_controller/command", 1);
    pub_joint_cmd[4] = nh_.advertise<unitree_legged_msgs::MotorCmd>(robot_name_ + "_gazebo" + "/FR_thigh_controller/command", 1);
    pub_joint_cmd[5] = nh_.advertise<unitree_legged_msgs::MotorCmd>(robot_name_ + "_gazebo" + "/FR_calf_controller/command", 1);
    pub_joint_cmd[6] = nh_.advertise<unitree_legged_msgs::MotorCmd>(robot_name_ + "_gazebo" + "/RL_hip_controller/command", 1);
    pub_joint_cmd[7] = nh_.advertise<unitree_legged_msgs::MotorCmd>(robot_name_ + "_gazebo" + "/RL_thigh_controller/command", 1);
    pub_joint_cmd[8] = nh_.advertise<unitree_legged_msgs::MotorCmd>(robot_name_ + "_gazebo" + "/RL_calf_controller/command", 1);
    pub_joint_cmd[9] = nh_.advertise<unitree_legged_msgs::MotorCmd>(robot_name_ + "_gazebo" + "/RR_hip_controller/command", 1);
    pub_joint_cmd[10] = nh_.advertise<unitree_legged_msgs::MotorCmd>(robot_name_ + "_gazebo" + "/RR_thigh_controller/command", 1);
    pub_joint_cmd[11] = nh_.advertise<unitree_legged_msgs::MotorCmd>(robot_name_ + "_gazebo" + "/RR_calf_controller/command", 1);

    sub_joint_msg[0] = nh_.subscribe(robot_name_ + "_gazebo" + "/FL_hip_controller/state", 2, &GazeboDog::FL_hip_state_callback, this);
    sub_joint_msg[1] = nh_.subscribe(robot_name_ + "_gazebo" + "/FL_thigh_controller/state", 2, &GazeboDog::FL_thigh_state_callback, this);
    sub_joint_msg[2] = nh_.subscribe(robot_name_ + "_gazebo" + "/FL_calf_controller/state", 2, &GazeboDog::FL_calf_state_callback, this);
    sub_joint_msg[3] = nh_.subscribe(robot_name_ + "_gazebo" + "/FR_hip_controller/state", 2, &GazeboDog::FR_hip_state_callback, this);
    sub_joint_msg[4] = nh_.subscribe(robot_name_ + "_gazebo" + "/FR_thigh_controller/state", 2, &GazeboDog::FR_thigh_state_callback, this);
    sub_joint_msg[5] = nh_.subscribe(robot_name_ + "_gazebo" + "/FR_calf_controller/state", 2, &GazeboDog::FR_calf_state_callback, this);
    sub_joint_msg[6] = nh_.subscribe(robot_name_ + "_gazebo" + "/RL_hip_controller/state", 2, &GazeboDog::RL_hip_state_callback, this);
    sub_joint_msg[7] = nh_.subscribe(robot_name_ + "_gazebo" + "/RL_thigh_controller/state", 2, &GazeboDog::RL_thigh_state_callback, this);
    sub_joint_msg[8] = nh_.subscribe(robot_name_ + "_gazebo" + "/RL_calf_controller/state", 2, &GazeboDog::RL_calf_state_callback, this);
    sub_joint_msg[9] = nh_.subscribe(robot_name_ + "_gazebo" + "/RR_hip_controller/state", 2, &GazeboDog::RR_hip_state_callback, this);
    sub_joint_msg[10] = nh_.subscribe(robot_name_ + "_gazebo" + "/RR_thigh_controller/state", 2, &GazeboDog::RR_thigh_state_callback, this);
    sub_joint_msg[11] = nh_.subscribe(robot_name_ + "_gazebo" + "/RR_calf_controller/state", 2, &GazeboDog::RR_calf_state_callback, this);
...
bool GazeboDog::main_update(double dt)
{
    ...
    {
        dog_control_.process_quadruped_cmd(state_, dt);
    }
...
bool GazeboDog::send_cmd()
{
    ...
    for (int i = 0; i < NUM_DOF; i++)
    {
        ...
        // std::cout << "sending command:" << low_cmd.motorCmd[i] << std::endl;
...
void GazeboDog::quadruped_cmd_callback(const quadruped_msgs::QuadrupedMove &msg)
{
    state_.ctrl.quadruped_cmd = msg;
    state_.ctrl.quadruped_cmd_received = true;
}
