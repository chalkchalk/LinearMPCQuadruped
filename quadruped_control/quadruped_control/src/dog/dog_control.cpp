...
DogControl::DogControl(): robot_log_(ros::package::getPath("quadruped_control") + "/logs/")
{
    ros::NodeHandle _nh;
...
bool DogControl::stand_ctrl(DogState &state)
{
    ...
    {
        if (state.ctrl.quadruped_cmd.cmd_code == state.ctrl.quadruped_cmd.CMD_LAY_DOWN)
        {
            state.ctrl.stand_status = 1;
            // std::cout << "laying down" << std::endl;
        }
        else if (state.ctrl.quadruped_cmd.cmd_code == state.ctrl.quadruped_cmd.CMD_STAND_UP)
        {
...
void DogControl::process_walk_mode_cmd(DogState &state, double dt)
{
    ...
    if(!state.ctrl.quadruped_cmd_last.is_walking)
    {
        state.ctrl.com_pos_set = state.attitude.com_pos;
    }
    Eigen::Vector3d new_ang_cmd;
    Eigen::Vector3d new_lin_cmd;
    if (state.ctrl.quadruped_cmd.proportional_cmd == true)
    {
        new_ang_cmd = Eigen::Vector3d(0, 0, state.ctrl.quadruped_cmd.yaw_rate * gait_ptr_->get_max_vel()(2));
        new_lin_cmd = Eigen::Vector3d(state.ctrl.quadruped_cmd.vel_x * gait_ptr_->get_max_vel()(0), state.ctrl.quadruped_cmd.vel_y * gait_ptr_->get_max_vel()(1), 0);
    }
    else
    {
        new_ang_cmd = Eigen::Vector3d(0, 0, Utils::clamp(state.ctrl.quadruped_cmd.yaw_rate, -gait_ptr_->get_max_vel()(2), gait_ptr_->get_max_vel()(2)));
        new_lin_cmd = Eigen::Vector3d(Utils::clamp(state.ctrl.quadruped_cmd.vel_x, -gait_ptr_->get_max_vel()(0), gait_ptr_->get_max_vel()(0)),
                                      Utils::clamp(state.ctrl.quadruped_cmd.vel_y, -gait_ptr_->get_max_vel()(1), gait_ptr_->get_max_vel()(1)), 0);
    }
    ...
    if (state.ctrl.quadruped_cmd.proportional_attitude == true)
    {
        state.ctrl.com_pos_set(2) = gait_ptr_->get_body_height(state) + state.ctrl.quadruped_cmd.body_height * controller_ptr_->get_body_height_max_delta();
    }
    else
    {
        state.ctrl.com_pos_set(2) = Utils::clamp(state.ctrl.quadruped_cmd.body_height, gait_ptr_->get_body_height(state) - controller_ptr_->get_body_height_max_delta(), gait_ptr_->get_body_height(state) + controller_ptr_->get_body_height_max_delta());
    }
...
void DogControl::process_stance_mode_cmd(DogState &state, double dt)
{
    ...
    if (state.ctrl.quadruped_cmd.proportional_attitude == true)
    {
        state.ctrl.com_euler_set(0) = state.ctrl.quadruped_cmd.body_roll * controller_ptr_->get_max_angle()(0);
        state.ctrl.com_euler_set(1) = state.ctrl.quadruped_cmd.body_pitch * controller_ptr_->get_max_angle()(1);
        state.ctrl.yaw_offset_set = state.ctrl.quadruped_cmd.body_yaw * controller_ptr_->get_max_angle()(2);
        state.ctrl.com_pos_set(2) = gait_ptr_->get_body_height(state) + state.ctrl.quadruped_cmd.body_height * controller_ptr_->get_body_height_max_delta();
    }
    else
    {
        state.ctrl.com_euler_set(0) = Utils::clamp(state.ctrl.quadruped_cmd.body_roll, -controller_ptr_->get_max_angle()(0), controller_ptr_->get_max_angle()(0));
        state.ctrl.com_euler_set(1) = Utils::clamp(state.ctrl.quadruped_cmd.body_pitch, -controller_ptr_->get_max_angle()(1), controller_ptr_->get_max_angle()(1));
        state.ctrl.yaw_offset_set = Utils::clamp(state.ctrl.quadruped_cmd.body_yaw, -controller_ptr_->get_max_angle()(2), controller_ptr_->get_max_angle()(2));
        state.ctrl.com_pos_set(2) = Utils::clamp(state.ctrl.quadruped_cmd.body_height, gait_ptr_->get_body_height(state) - controller_ptr_->get_body_height_max_delta(), gait_ptr_->get_body_height(state) + controller_ptr_->get_body_height_max_delta());
    }
    if (state.attitude.com_pos(2) > 0.12) // terrain_adaptaion
    {
...
void DogControl::process_gait_switch_cmd(DogState &state, double dt)
{
    ...
    if (state.ctrl.quadruped_cmd.cmd_code == state.ctrl.quadruped_cmd.CMD_GAIT_SWITCH_1)
    {
...
void DogControl::process_quadruped_cmd(DogState &state, double dt)
{
    if (!state.ctrl.quadruped_cmd_received)
    {
        return;
    }

    if (state.ctrl.quadruped_cmd.is_walking)
    {
    ...
    gait_ptr_->gait_cmd_handler(state.ctrl.quadruped_cmd.gait_code);

    state.ctrl.com_euler_set(2) = Utils::periodical_clamp(state.ctrl.com_euler_set(2), -PI, PI, 2 * PI);
    state.ctrl.quadruped_cmd_last = state.ctrl.quadruped_cmd;
}
