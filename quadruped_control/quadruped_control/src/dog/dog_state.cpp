...
void AttitudeState::reset()
{
    ...
    hip_pos_abs.setZero();
    com_ang_vel_r_filtered.setZero();
...
void CtrlState::reset()
{
    ...
    com_set.setZero();
    yaw_offset_set = 0;
    quadruped_cmd.cmd_code = quadruped_cmd.CMD_EMPTY;
    stand_status = 0;
    udp_online = true;
    reset_from_rosparam();
}
...
void DogState::contact_estimate()
{
    ...
    for (int i = 0; i < 4; ++i)
    {
        attitude.is_contact[i] = is_prone || attitude.foot_force[i] > ctrl.foot_contact_threshold; // for foot sensor only

        // attitude.is_contact[i] = is_prone || ctrl.plan_contact[i];
