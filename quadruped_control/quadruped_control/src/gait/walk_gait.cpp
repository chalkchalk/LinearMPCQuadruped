...
void WalkGait::counter_contact_plan(DogState &state)
{
    if (moving_plan)
    {
        if (!(is_moving[0] || is_moving[1] || is_moving[2] || is_moving[3])) // wait untill all legs stop(to finish the unfinshed gait of some legs)
        {
            for (int i = 0; i < NUM_LEG; ++i)
            {
                is_moving[i] = true;
...
{
    if(cmd == quadruped_msgs::QuadrupedMove::GAIT_MOVE_1)
    {
        initial_phase << 0.0, 0.5, 0.5, 0.0;
    }
    else if(cmd == quadruped_msgs::QuadrupedMove::GAIT_MOVE_2)
    {
